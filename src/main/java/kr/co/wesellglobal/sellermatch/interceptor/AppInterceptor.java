...
public class AppInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		SNSLogin snsLoginNaver = new SNSLogin(naverSns); SNSLogin snsLoginKakao = new SNSLogin(kakaoSns); 
		OAuth2Operations oauthOperations = googleConnectionFactory.getOAuthOperations(); 
		String googleUrl = oauthOperations.buildAuthorizeUrl(GrantType.AUTHORIZATION_CODE, googleOAuth2Parameters);		
		
		Cookie naver_check = getCookie(request,"naver_url");
		Cookie kakao_check = getCookie(request,"kakao_url");
		Cookie google_check = getCookie(request,"google_url");
		if (naver_check == null) {
			Cookie naver_url = new Cookie("naver_url", snsLoginNaver.getAuthURL()); // 쿠키생성
			response.addCookie(naver_url);
		}
		if (kakao_check == null) {
			Cookie kakao_url = new Cookie("kakao_url", snsLoginKakao.getAuthURL()); //쿠키 생성 
			response.addCookie(kakao_url);
		}
		if (google_check == null) {
			Cookie google_url = new Cookie("google_url", googleUrl); // 쿠키 생성
			response.addCookie(google_url);
		}
